...
std::vector<match_result> mult_par_finder(
    ...
    for (size_t q = 0; q < num_queries; q++) {
        ...
        // Time series loop
        #pragma omp parallel{
        match_result loc_global_best;
        loc_global_best.distance = std::numeric_limits<real_t>::max();
        loc_global_best.series_id = -1;
        loc_global_best.start_index = 0;
...
#pragma omp simd reduction(+:ex, ex2)
            ...
            if (local_best.distance < loc_global_best) {
                loc_global_best = local_best;
            }

        }
        // Definitely saving the overall winner for the current query
        if (loc_global_best < best_results[q]) {
#pragma omp critical
            best_results[q] = loc_global_best;
        }
    }
    }
